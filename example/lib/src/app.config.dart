...
class MyAppConfigKeys {static final routes = _RouteKeys();
...
class MyAppGeneratedThemeExtension extends ConfigTheme<MyAppGeneratedThemeExtension> {MyAppGeneratedThemeExtension({required super.themeMap});
...
Color get storeFrontBg => _ColorUtil.parseColorValue( themeMap['colors']?['storeFrontBg'] );
double get homeTitleSize => themeMap['sizes']?['homeTitleSize'];
double get detailTitleSize => themeMap['sizes']?['detailTitleSize'];
@override MyAppGeneratedThemeExtension copyWith({Map<String, Map<String, dynamic>>? themeMap}) { this.themeMap.addAll( themeMap ?? {} );
return MyAppGeneratedThemeExtension( themeMap: this.themeMap ); } 
@override MyAppGeneratedThemeExtension lerp(ThemeExtension<MyAppGeneratedThemeExtension>? other, double t, ) { if ( other is! MyAppGeneratedThemeExtension) {
return this;
}
...
themeMap['colors']!['storeFrontBg'] = _ColorUtil.colorToString( Color.lerp( storeFrontBg, other.storeFrontBg, t )! );
...
Map<String, bool> get map => {MyAppConfigKeys.flags.isEnabled: false, MyAppConfigKeys.flags.andThis: true, MyAppConfigKeys.flags.andThat: true, MyAppConfigKeys.flags.orThis: false, MyAppConfigKeys.flags.orThat: false, MyAppConfigKeys.flags.showTitle: true};
...
Map<String, String> get map => {MyAppConfigKeys.images.loginHeaderImage: 'https://pub.dev/static/hash-qr9i96gp/img/pub-dev-logo-2x.png', MyAppConfigKeys.images.storeFrontHeaderImage: 'https://pub.dev/static/hash-qr9i96gp/img/pub-dev-logo-2x.png'};
String get loginHeaderImage => map[ MyAppConfigKeys.images.loginHeaderImage ] ?? '/';
String get storeFrontHeaderImage => map[ MyAppConfigKeys.images.storeFrontHeaderImage ] ?? '/';
 }
class _ImageAccessor {const _ImageAccessor(this.config);

final Configuration config;

String get loginHeaderImage => config.image( MyAppConfigKeys.images.loginHeaderImage );
String get storeFrontHeaderImage => config.image( MyAppConfigKeys.images.storeFrontHeaderImage );
 }
...
Map<String, String> get map => {MyAppConfigKeys.colors.primary: 'EFF1F3', MyAppConfigKeys.colors.secondary: 'CC0000', MyAppConfigKeys.colors.tertiary: '000000', MyAppConfigKeys.colors.storeFrontBg: 'EFF1F3'};
...
Color get storeFrontBg => config.colorValue( MyAppConfigKeys.colors.storeFrontBg );
 }
...
Map<String, double> get map => {MyAppConfigKeys.sizes.homeTitleSize: 14.0, MyAppConfigKeys.sizes.detailTitleSize: 22.0};
double get homeTitleSize => map[ MyAppConfigKeys.sizes.homeTitleSize ] ?? 0.0;
double get detailTitleSize => map[ MyAppConfigKeys.sizes.detailTitleSize ] ?? 0.0;
 }
...
double get detailTitleSize => config.size( MyAppConfigKeys.sizes.detailTitleSize );
 }
...
class _I18nDartDe implements _I18nDartEn {
	...
	@override String get currentScope => 'Aktuellen Umfang';
}
...
extension on _I18nDartEn {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'currentScope': return 'Current Scope';
			default: return null;
		}
	}
}
...
extension on _I18nDartDe {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'currentScope': return 'Aktuellen Umfang';
			default: return null;
		}
	}
}
...
class MyAppGeneratedScope extends ConfigScope {@override String name = '__GeneratedScope';
