...
class ThemeWriter extends Writer {
  ...
  ThemeWriter(String elementName, this._yamlConfiguration)
      : name = 'Generated${elementName}ThemeExtension';
  ...
  Constructor _buildConstructor() {
    return Constructor((cBuilder) {
      cBuilder.optionalParameters.add(Parameter((builder) {
        builder
          ..name = 'themeMap'
          ..required = true
          ..toSuper = true
    ...
    return _yamlConfiguration.colors.map((e) {
      return Method((builder) {
        builder
          ...
          ..lambda = true
          ..body = Code(
              'ColorParser.parse( themeMap[\'colors\']?[\'${(e.name as String).canonicalize}\'] )');
      });
    }).toList();
  }
    ...
    return _yamlConfiguration.sizes.map((e) {
      return Method((builder) {
        builder
          ...
          ..lambda = true
          ..body = Code(
              'themeMap[\'sizes\']?[\'${(e.name as String).canonicalize}\']');
      });
    }).toList();
  }
        ...
        ..body = Block((builder) {
          builder.statements.addAll([
            ...
            ..._yamlConfiguration.colors.map((e) {
              return Code(
                  'themeMap[\'colors\']![\'${(e.name as String).canonicalize}Color\'] = ColorParser.colorToString( Color.lerp( ${(e.name as String).canonicalize}Color, other.${(e.name as String).canonicalize}Color, t )! );');
            }),
            ..._yamlConfiguration.sizes.map((e) {
              return Code(
                  'themeMap[\'sizes\']![\'${(e.name as String).canonicalize}Size\'] = lerpDouble( ${(e.name as String).canonicalize}Size, other.${(e.name as String).canonicalize}Size, t);');
            }),
