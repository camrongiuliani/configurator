  ...
  group('Yaml Load Tests', () {
    ...
    test('Split Yaml Reduce Test', () {
      expect(() {
        ...
        List<YamlConfiguration> configs =
            [m1, m2].map((e) => YamlParser.fromYamlString(e)).toList();
    ...
    test('Split Yaml Reduce Results Unique Test', () {
      ...
      YamlConfiguration config =
          configs.reduce((value, element) => value + element);

      expect(max(configs[0].colors.length, configs[1].colors.length),
          equals(config.colors.length));
      expect(max(configs[0].flags.length, configs[1].flags.length),
          equals(config.flags.length));
      expect(max(configs[0].routes.length, configs[1].routes.length),
          equals(config.routes.length));
      expect(max(configs[0].images.length, configs[1].images.length),
          equals(config.images.length));
      expect(max(configs[0].strings.length, configs[1].strings.length),
          equals(config.strings.length));
      expect(max(configs[0].misc.length, configs[1].misc.length),
          equals(config.misc.length));
      expect(max(configs[0].padding.length, configs[1].padding.length),
          equals(config.padding.length));
      expect(max(configs[0].margins.length, configs[1].margins.length),
          equals(config.margins.length));
      expect(max(configs[0].sizes.length, configs[1].sizes.length),
          equals(config.sizes.length));
    });
    ...
    test('Invalid Yaml Throws', () {
      ...
      // Int ID throws
      expect(() {
        var m = File(testYaml1)
            .readAsStringSync()
            .replaceAll('id: app_scope', 'id: 1');
        YamlParser.fromYamlString(m);
      }, invalidYamlMatcher);
      ...
      expect(() {
        var m = File(testYaml1)
            .readAsStringSync()
            .replaceAll('configuration:', 'dd:');
        YamlParser.fromYamlString(m);
      }, invalidYamlMatcher);

      // Bad Configuration key throws
      expect(() {
        var m = File(badConfigYaml)
            .readAsStringSync()
            .replaceAll('configuration:', 'dd:');
        YamlParser.fromYamlString(m);
      }, invalidYamlMatcher);
